...
def test_handle_tournaments_in_group_uses_compact_public_view():
    """Same command name, different behaviour by chat type
    (CLAUDE_TASK_BOT_RU_GROUPS.md, раздел 3): a group /tournaments never
    takes a status filter and never paginates."""
    from bot.handlers.tournaments import handle_tournaments

    message = _fake_message(text="/tournaments")
    message.chat.type = "group"
    active = _paged([{"id": 1, "name": "Active Cup", "status": "active", "started_at": "2026-01-01T00:00:00+00:00"}])
    with patch("bot.handlers.tournaments.get_tournaments", return_value=active) as mock_get, \
         patch("bot.telegram_bot.bot.send_message") as mock_send:
        handle_tournaments(message)

    assert "Active Cup" in mock_send.call_args[0][1]
    for call_kwargs in (c.kwargs for c in mock_get.call_args_list):
        assert "status" in call_kwargs


def test_handle_tournaments_in_group_rate_limited():
    from bot.handlers.tournaments import handle_tournaments

    message = _fake_message(text="/tournaments")
    message.chat.type = "group"
    with patch("bot.handlers.tournaments.check_group_command_rate_limit", return_value=(False, True)), \
         patch("bot.handlers.tournaments.get_tournaments") as mock_get, \
         patch("bot.telegram_bot.bot.send_message") as mock_send:
        handle_tournaments(message)

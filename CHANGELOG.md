...
### Added — Полная русификация + разделение личных/групповых команд (CLAUDE_TASK_BOT_RU_GROUPS.md)

- `bot/i18n.py` (новый) — единый слой перевода/форматирования: словари
  для каждого MS-enum'а (статусы турнира/сезона/серии/драфта, тип
  турнира, сторона победы, роль, категория магазина, редкость, категория
  достижения, тип титула, источник монет), `tr()` с безопасным
  фоллбэком, монеты/баллы/проценты/даты/склонения количества.
  `tests/test_i18n.py` (19 тестов) параметризованно проверяет каждое
  реальное значение каждого enum'а MS — единственный тест в проекте,
  явно защищающий от регресса "забыли перевести новое значение".
  Переписаны все presenters (`shop`, `fantasy`, `tournaments`, `seasons`,
  `games`, `history`, `profile`, `hub`, `players`, `gifts`, `stats`,
  `economy`, `vs`, `ratings`, `notifications`, `menu`) — устранены сырые
  английские статусы, "Fantasy"→"Фэнтези", "ELO"→"Эло", баг с ₽ на
  монетах, нестабильное число знаков после запятой, формальное "Вы" там,
  где остальной бот на "ты".
- `bot/chat_policy.py` (новый) + `guarded_callback(bot, private_only=True)`
  (`bot/dispatch.py`) — единая точка решения "разрешён ли этот
  персональный/мутирующий сценарий в этом чате", проверяется на КАЖДОМ
  callback (не только на команде-точке-входа — пересланное в группу
  сообщение с личным меню не должно ожить там). Применено ко всем
  персональным экранам (баланс, инвентарь, подарки, достижения/титулы,
  настройки, весь инлайн-навигационный главный экран) и всем мутирующим
  действиям (покупка, Fantasy-драфт, отправка подарка, отвязка аккаунта,
  вход в любой FSM со свободным вводом).
- `bot/storage.py` — FSM (`fsm_state`) переведён на составной ключ
  `(chat_id, telegram_user_id)`; встроенная идемпотентная миграция схемы
  при старте переносит старые строки с `telegram_user_id = chat_id`.
- `bot/presenters/group.py` + `bot/handlers/group_commands.py` (новые) —
  семь безопасных публичных команд общего чата: `/top`, `/season`,
  `/tournaments`, `/player <ник>`, `/compare <ник1> | <ник2>`, `/game
  <номер>`, `/help` — ничего личного, все данные уже были публичными на
  сайте. `/tournaments` и `/compare` делят имя со старой личной командой,
  ветвятся по `chat.type` в существующих `handlers/tournaments.py` /
  `handlers/compare.py` (числовой `/compare <id>` остаётся, но помечен
  deprecated и работает только в личном чате — зависит от привязанного
  аккаунта вызывающего). Кнопки "подробнее" на групповых экранах — всегда
  настоящий deep-link `t.me/<bot>?start=...` в личный чат
  (`bot/bot_identity.py`, новый), никогда `callback_data` на
  `private_only` хендлер. `/start` в группе — короткое сообщение без
  резолва привязки + кнопка "Открыть личный кабинет"; в личном чате —
  без изменений.
- `bot/ratelimit.py::check_group_command_rate_limit` — антиспам новых
  команд, два окна на `(chat_id, user_id)`, применяется только в
  group/supergroup.
- `bot/commands.py` (новый) + `flask set-commands` (`run.py`) —
  идемпотентная регистрация `/`-меню Telegram по scope: `/start`+`/help`
  для личных чатов, все семь публичных команд для групповых, с русскими
  описаниями.
- `bot/keyboards/nav.py::cb()` теперь **бросает** `CallbackDataTooLong`
  при превышении 64-байтового лимита Telegram вместо тихого лога — баг
  ловится сразу на месте, а не мёртвой кнопкой в проде.
- MS: `/api/v1/bot/players/<id>` теперь отдаёт публичную рейтинг-карточку
  (`rank`/`games_played`/`games_won`/`win_rate`, 0/None вместо 404 для
  игрока без сыгранных игр); `/api/v1/bot/history` отдаёт
  `total`/`total_pages`/`has_next` (бот больше не гадает по размеру
  страницы); `/api/v1/bot/shop/items/<id>` отдаёт `can_buy`/
  `unavailable_reason`, посчитанные на сервере теми же правилами, что и
  сама покупка — бот не дублирует бизнес-логику доступности.
- Тесты: 352 (было 303) в MS-TG, 61 в MS — все проходят.

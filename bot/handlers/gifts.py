"""
Gift-sending FSM. State lives in bot/storage.py (SQLite), keyed by
(chat_id, telegram_user_id) — private-only (see private_only=True below),
...
def _state_step(chat_id: int, telegram_user_id: int, step: str) -> bool:
    state = storage.get_fsm_state(chat_id, telegram_user_id)
...
def handle_gift_pick_item(call) -> None:
    ...
    storage.set_fsm_state(
        call.message.chat.id, telegram_id, SCENARIO, "await_recipient",
...
@bot.message_handler(
    func=lambda m: _state_step(m.chat.id, m.from_user.id, "await_recipient"), content_types=["text"],
)
...
def handle_gift_recipient_pick(call) -> None:
    telegram_id = call.from_user.id
    state = storage.get_fsm_state(call.message.chat.id, telegram_id)
    ...
    storage.set_fsm_state(call.message.chat.id, telegram_id, SCENARIO, "await_note", data)
...
def _show_confirm(chat_id: int, telegram_user_id: int) -> None:
    # Private chats only (guarded above) — telegram_user_id doubles as the
    # player's own telegram_id for the API calls below.
    state = storage.get_fsm_state(chat_id, telegram_user_id)
    data = state["data"]

    giftable = get_giftable_items(api_client, telegram_user_id)
    ...
    storage.set_fsm_state(chat_id, telegram_user_id, SCENARIO, "confirm", data)
...
def handle_gift_skip_note(call) -> None:
    telegram_id = call.from_user.id
    if not _state_step(call.message.chat.id, telegram_id, "await_note"):
        _edit(call, *build_gifts_hub_message())
        return
    _show_confirm(call.message.chat.id, telegram_id)


@bot.message_handler(func=lambda m: _state_step(m.chat.id, m.from_user.id, "await_note"), content_types=["text"])
def handle_gift_note_text(message) -> None:
    note = (message.text or "").strip()[:MAX_NOTE_LENGTH]
    telegram_id = message.from_user.id
    state = storage.get_fsm_state(message.chat.id, telegram_id)
    data = state["data"]
    data["note"] = note
    storage.set_fsm_state(message.chat.id, telegram_id, SCENARIO, "await_note", data)
    _show_confirm(message.chat.id, telegram_id)
